...
export default function SuperAdminDashboardPage() {
  ...
  function getSubscriptionStatus(business) {
    if (!business.active || business.deleted) {
      return { label: "Suspended", classes: "bg-red-100 text-red-700" };
    }

    if ((business.subscriptionPrice || 0) > 0) {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <DashboardHeader monthlyRevenue={monthlyRevenue} />
...
function DashboardHeader({ monthlyRevenue }) {
  return (
      ...
      <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        ...
        <div className="flex items-center gap-3 bg-white/10 px-4 py-3 rounded-xl backdrop-blur">
          ...
          <div>
            ...
            <p className="font-semibold">${monthlyRevenue.toFixed(2)}</p>
          </div>
        </div>
      </div>
    </div>
...
function BusinessesTable({ businesses, getStatus }) {
  return (
            ...
            {businesses.map((biz) => {
              ...
              return (
                <tr key={biz.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-4">
                    <span
                      className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${status.classes}`}
                    >
                      {status.label}
                    </span>
                  </td>
...
function SidebarSummary({ totalUsers, businessAdmins }) {
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-semibold">{businessAdmins}</span>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        ...
        <p className="text-xs text-gray-500 mb-3">
          View all subscriptions and expiring businesses.
        </p>
        <button
        ...
        >
          Open Subscriptions <ArrowRight size={14} />
        </button>
      </div>
    </div>
  );
